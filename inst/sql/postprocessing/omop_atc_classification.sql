...
BEGIN
	...
	IF requires_processing THEN
		...
		SELECT DISTINCT
		  ...
		  FROM {target_schema}.concept c2
		  INNER JOIN (
		    SELECT *
		    FROM {target_schema}.concept_relationship
		    WHERE relationship_id = 'Subsumes' AND invalid_reason IS NULL) cr
		  ON cr.concept_id_2 = c2.concept_id
		  INNER JOIN (
		    SELECT * FROM {target_schema}.concept
		    WHERE
		...
		FROM {target_schema}.concept c2
		INNER JOIN (SELECT * FROM {target_schema}.concept_relationship WHERE relationship_id = 'Subsumes' AND invalid_reason is null) cr
		ON cr.concept_id_2 = c2.concept_id
		INNER JOIN (SELECT * FROM {target_schema}.concept WHERE vocabulary_id = 'ATC' AND concept_class_id = 'ATC 2nd' AND invalid_reason is null) c1
		ON cr.concept_id_1 = c1.concept_id
		WHERE
		 c2.vocabulary_id = 'ATC' AND
		...
		INNER JOIN (SELECT * FROM {target_schema}.concept_relationship WHERE relationship_id = 'Subsumes' AND invalid_reason is null) cr
		ON cr.concept_id_2 = c2.concept_id
		INNER JOIN (SELECT * FROM {target_schema}.concept WHERE vocabulary_id = 'ATC' AND concept_class_id = 'ATC 4th' AND invalid_reason is null) c1
		ON cr.concept_id_1 = c1.concept_id
		WHERE
		 c2.vocabulary_id = 'ATC' AND
		...
		INNER JOIN
		    (SELECT * FROM {target_schema}.concept_relationship WHERE relationship_id = 'ATC - RxNorm pr lat' AND invalid_reason is null) cr
		ON cr.concept_id_1 = d.atc_5th_id
		INNER JOIN (SELECT * FROM {target_schema}.concept WHERE vocabulary_id = 'RxNorm' AND concept_class_id = 'Ingredient' AND invalid_reason is null) c2
		  ON cr.concept_id_2 = c2.concept_id
		INNER JOIN (
		  SELECT DISTINCT
		  ...
		  FROM {target_schema}.concept c1
		  INNER JOIN {target_schema}.concept_ancestor ca
		  ON ca.ancestor_concept_id = c1.concept_id
		  INNER JOIN (SELECT * FROM {target_schema}.concept WHERE vocabulary_id = 'RxNorm' AND invalid_reason is null) c2
		...
		INNER JOIN {target_schema}.concept pin
		...
		LEFT JOIN {target_schema}.concept_relationship cr
		ON cr.concept_id_2 = tmp1.in_pin_min_id
		INNER JOIN {target_schema}.concept ming
