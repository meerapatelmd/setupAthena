...
atc_classes <-
query(sql_statement = "SELECT * FROM omop_vocabulary.concept WHERE vocabulary_id = 'ATC' AND standard_concept = 'C' AND invalid_reason IS NULL;")
atc_classes
...
rxnorm_individuals <-
query(sql_statement = "SELECT * FROM omop_vocabulary.concept WHERE vocabulary_id IN ('RxNorm', 'RxNorm Extension') AND standard_concept <> 'C' AND concept_class_id = 'Ingredient' AND invalid_reason IS NULL;")
rxnorm_individuals
